  ...
  return (
    <>
      ...
      <ThemeContainer title='常规主题'>
        <GeneralTheme data={data} theme={theme} />
      </ThemeContainer>
      <ThemeContainer title='扩展色'>
        <CustomVariables data={data} theme={theme} popView={popView}/>
      </ThemeContainer>
    </>
  )
}
...
function GeneralTheme ({ data, theme }) {
  ...
  const onColorChange = useCallback(({ id, color, items }: { id, color, items?}) => {
    ...
    if (Array.isArray(items)) {
      generateColors({ data, theme, primaryColor: color, children: items })
    }
