...
import { convertHyphenToCamel, convertCamelToHyphen, useThemeChange } from ".."
...
import { ThemeGlobalContext } from "..";
...
const ThemeList = ({ onChange, theme, setTitle }: any) => {
  ...
  const { generateDefaultVariables } = useContext(ThemeGlobalContext);
...
function GeneralTheme ({ variable, onBrandColorChange, onThemeChange }: any) {
  const { mybricksPrefixCls } = useContext(ThemeGlobalContext);
